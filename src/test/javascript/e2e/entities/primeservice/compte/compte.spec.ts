...
describe('Compte e2e test', () => {
  ...
  it('should load Comptes', async () => {
    ...
    expect(await compteComponentsPage.getTitle()).to.eq('primegatewayApp.primeserviceCompte.home.title');
  ...
  it('should load create Compte page', async () => {
    ...
    expect(await compteUpdatePage.getPageTitle()).to.eq('primegatewayApp.primeserviceCompte.home.createOrEditLabel');
  ...
  it('should create and save Comptes', async () => {
    ...
    await promise.all([
      compteUpdatePage.setNumeroCompteInput('numeroCompte'),
      compteUpdatePage.agentSelectLastOption(),
      compteUpdatePage.banqueSelectLastOption()
    ]);

    expect(await compteUpdatePage.getNumeroCompteInput()).to.eq('numeroCompte', 'Expected NumeroCompte value to be equals to numeroCompte');
    const selectedStatut = compteUpdatePage.getStatutInput();
    if (await selectedStatut.isSelected()) {
      await compteUpdatePage.getStatutInput().click();
      expect(await compteUpdatePage.getStatutInput().isSelected(), 'Expected statut not to be selected').to.be.false;
    } else {
      await compteUpdatePage.getStatutInput().click();
      expect(await compteUpdatePage.getStatutInput().isSelected(), 'Expected statut to be selected').to.be.true;
    }
  ...
  it('should delete last Compte', async () => {
    ...
    expect(await compteDeleteDialog.getDialogTitle()).to.eq('primegatewayApp.primeserviceCompte.delete.question');
